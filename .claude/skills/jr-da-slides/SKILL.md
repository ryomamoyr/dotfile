...
## 🚨 絶対ルール（生成前・保存前に必ずチェック）

以下は逸脱不可。「情報量が多くて入らない」を理由にサイズを下げない（サイズを保ち、内容を絞るか別スライドに分割する）。

### フォントサイズ最小値

| 種別 | 最小サイズ | 例 |
|------|----------|-----|
| **本文・箇条書き** | **14pt** | `add_textbox(..., size_pt=14)` |
| **テーブル本文** | **14pt** | `body_size=14` |
| **テーブルヘッダー** | **14pt** Bold | `header_size=14` |
| **チャートラベル** | **14pt** | matplotlib `fontsize=14` |
| **コード/等幅フォント(Menlo等)** | **11pt** | `{"text": "...", "size": 11, "font": "Menlo"}` |
| **出典・注釈のみ** | **10pt** | 唯一14pt未満が許容される要素 |

### ❌ 禁止パターン（過去にやらかした例）

```python
# 禁止: 本文・テーブルで14pt未満
add_textbox(..., size_pt=11)            # ❌
add_textbox(..., size_pt=13)            # ❌
add_table_with_data(..., body_size=11)  # ❌
add_table_with_data(..., body_size=12)  # ❌

# 禁止: ターミナル風ボックス内のコードで11pt未満
{"text": "$ git log", "size": 9, "font": "Menlo"}   # ❌
{"text": "$ git log", "size": 10, "font": "Menlo"}  # ❌
```

### ✅ 正しいパターン

```python
add_textbox(..., size_pt=14)                                   # OK
add_textbox(..., size_pt=16, bold=True)                        # OK (見出し)
...
### その他の絶対ルール
- **プレースホルダー**: レイアウトのタイトル等は必ずプレースホルダー使用。テキストボックスで代替しない
- **フォント**: 游ゴシック統一（コードのみ Menlo 等の等幅フォント可）
- **テーマカラー外の色追加禁止**: design-system.md のパレットのみ

### 保存前セルフチェック
スライド生成後、保存前に以下を口頭で確認：
1. 全 `size_pt=` の値が 14 以上か？（コード/注釈以外）
2. 全 `body_size=` / `header_size=` の値が 14 以上か？
3. Menlo等の等幅フォント箇所が 11pt 以上か？
...
2. **タイトル・見出しはレイアウトのプレースホルダーを使用する（必須）**
3. コンテンツ配置: デザインシステムのコンポーネント関数を使用

#### プレースホルダー使用ルール（重要）

レイアウトが定義するプレースホルダー（タイトル、セクション名、サブタイトル等）は**必ずそのまま使用**し、テキストボックスで代替しないこと。

```python
# ✅ 正しい: プレースホルダーに直接テキストを設定
slide = prs.slides.add_slide(prs.slide_layouts[Layout.LEVEL1])
slide.placeholders[0].text = "分析結果のサマリー"

# ❌ 誤り: プレースホルダーを無視/削除してテキストボックスを追加
slide.shapes.add_textbox(left, top, w, h)  # タイトル位置に手動配置
```

**理由:**
- プレースホルダーはレイアウトからフォント・サイズ・行間・配置を自動継承する
- テキストボックスで代替するとテーマ継承が切れ、書式を手動で再現する必要が生じる
- プレースホルダーとテキストボックスが重なるとテンプレ文字列が二重表示される

**不要なプレースホルダーの扱い:**
- 使わないプレースホルダーはテキストを空にする（`ph.text = ""`）
- 削除（`remove`）はしない — 将来の編集時にレイアウト構造が壊れる

**テキストボックスを使う場面:**
- プレースホルダーが存在しないコンテンツ領域（テーブル横の注釈、KPIカード内テキスト等）
- レイアウトが提供しない自由配置の要素のみ
...
| 本文・箇条書き | 游ゴシック | **14-16pt（最小14pt厳守）** |
| テーブルヘッダー | 游ゴシック Bold | **14pt（最小）** |
| テーブルセル | 游ゴシック | **14pt（最小）** |
| チャートラベル | 游ゴシック | **14pt（最小）** |
| コード/等幅 | Menlo 等 | **11pt（最小、ターミナル風ボックス内）** |
| 出典・注釈 | 游ゴシック | 10pt, グレー（唯一14pt未満が許容される要素） |

游ゴシックに統一する理由：テンプレートのフォントスキームが游ゴシックで定義されており、他のフォントを混ぜると見た目が不揃いになる。コード/コマンドは可読性のため Menlo 等の等幅フォントを使う。

> **🚨 上記の最小サイズは厳守。詳細とNGパターンは冒頭の「絶対ルール」セクションを参照。**
...
既存ファイルにスライドを追加する場合は、**そのファイルのレイアウト・テーマを優先**する：

1. **レイアウト確認**: `prs.slide_layouts` を走査し、レイアウト名・インデックス・プレースホルダー構成を確認する
2. **テーマフォント確認**: テーマの `majorFont` / `minorFont` を確認し、追加スライドのフォントを揃える
3. **既存スライドの書式を踏襲**: 既存スライドのタイトルshapeの `defRPr`（サイズ・フォント参照）を確認し、同じ書式で統一する
4. **プレースホルダー使用**: template.potx のレイアウトと異なる場合でも、そのファイルのレイアウトのプレースホルダーをそのまま使用する
